...
const load = async (uid: UID.ContentType, { oldVersions, newVersions }: LoadContext) => {
  ...
  await leao.db.transaction(async ({ trx }) => {
    const contentTypes = Object.values(leao.contentTypes) as Schema.ContentType[];
    const components = Object.values(leao.components) as Schema.Component[];

    for (const model of [...contentTypes, ...components]) {
      const dbModel = leao.db.metadata.get(model.uid);
      ...
      for (const attribute of Object.values(dbModel.attributes) as any) {
        ...
        const oldVersionsRelations = await leao.db
          .getConnection()
          .select('*')
          .from(joinTable.name)
        ...
        if (!model.options?.draftAndPublish) {
          ...
          const newVersionsRelations = await leao.db
            .getConnection()
            .select('*')
            .from(joinTable.name)
            .whereIn(targetColumnName, ids)
