...
const createContentAPI = (leao: Core.Leao) => {
  ...
  const sanitizer = sanitize.createAPISanitizers({
    ...
    get sanitizers() {
      return {
        input: leao.sanitizers.get('content-api.input'),
        output: leao.sanitizers.get('content-api.output'),
      };
    },
  });
  ...
  const validator = validate.createAPIValidators({
    ...
    get validators() {
      return {
        input: leao.validators.get('content-api.input'),
      };
    },
  });

  return {
    permissions: instantiatePermissionsUtilities(leao),
    getRoutesMap,
    sanitize: sanitizer,
    validate: validator,
  };
