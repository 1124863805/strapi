  ...
  return (
    createCommand('transfer')
      ...
      .addOption(
        new Option(
          '--from <sourceURL>',
          `URL of the remote Leao instance to get data from`
        ).argParser(parseURL)
      )
      .addOption(new Option('--from-token <token>', `Transfer token for the remote Leao source`))
      .addOption(
        new Option(
          '--to <destinationURL>',
          `URL of the remote Leao instance to send data to`
        ).argParser(parseURL)
      )
      .addOption(
        new Option('--to-token <token>', `Transfer token for the remote Leao destination`)
      )
          ...
          async (thisCommand) => {
            ...
            if (!thisCommand.opts().fromToken) {
              const answers = await inquirer.prompt([
                {
                  type: 'password',
                  message: 'Please enter your transfer token for the remote Leao source',
                  name: 'fromToken',
                },
              ]);
            ...
            await getCommanderConfirmMessage(
              'The transfer will delete all the local Leao assets and its database. Are you sure you want to proceed?',
          ...
          async (thisCommand) => {
            ...
            if (!thisCommand.opts().toToken) {
              const answers = await inquirer.prompt([
                {
                  type: 'password',
                  message: 'Please enter your transfer token for the remote Leao destination',
                  name: 'toToken',
                },
              ]);
            ...
            await getCommanderConfirmMessage(
              'The transfer will delete existing data from the remote Leao! Are you sure you want to proceed?',
              { failMessage: 'Transfer process aborted' }
            )(thisCommand);
          }
        )
