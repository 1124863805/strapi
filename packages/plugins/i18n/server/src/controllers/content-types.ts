...
const controller = {
  async getNonLocalizedAttributes(ctx) {
    ...
    } = leao.service('admin::constants');

    const modelDef = leao.contentType(model);
    ...
    const permissions = await leao.admin.services.permission.findMany({
      where: {
    ...
    const availableLocalesResult = await leao.plugins['content-manager']
