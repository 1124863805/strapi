...
export const createContentType = async (
  ...
  leao.eventHub.emit('content-type.create', { contentType: newContentType });
...
export const editContentType = async (
  ...
  if (newKind !== previousKind && newKind === 'singleType') {
    const entryCount = await leao.db.query(uid).count();
    if (entryCount > 1) {
  ...
  if (newKind !== previousKind) {
    const apiHandler = leao.plugin('content-type-builder').service('api-handler');
  ...
  leao.eventHub.emit('content-type.update', { contentType: updatedContentType });
...
export const deleteContentType = async (uid: UID.ContentType, defaultBuilder: any = undefined) => {
  ...
  leao.eventHub.emit('content-type.delete', { contentType });
