...
export default ({ leao }: { leao: Core.Leao }) => {
  const sanitizeOutput = async (data: unknown | unknown[], ctx: Context) => {
    const schema = leao.getModel(FILE_MODEL_UID);
    const { auth } = ctx.state;

    return leao.contentAPI.sanitize.output(data, schema, { auth });
  };
  ...
  const validateQuery = async (data: Record<string, unknown>, ctx: Context) => {
    ...
    return leao.contentAPI.validate.query(data, schema, { auth });
  };
  ...
  const sanitizeQuery = async (data: Record<string, unknown>, ctx: Context) => {
    ...
    return leao.contentAPI.sanitize.query(data, schema, { auth });
  };
