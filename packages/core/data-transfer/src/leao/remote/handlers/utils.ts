...
const hasHttpServer = () => {
  // during server restarts, leao may not have ever been defined at all, so we have to check it first
  return typeof leao !== 'undefined' && !!leao?.server?.httpServer;
};
...
const disableTimeouts = () => {
  ...
  leao.log.info('[Data transfer] Disabling http timeouts');
};
const resetTimeouts = () => {
  ...
  leao.log.info('[Data transfer] Restoring http timeouts');
  ...
  (options: HandlerOptions) => {
    ...
    return async (ctx: Context) => {
      const cb: WSCallback = (ws) => {
        ...
        const cannotRespondHandler = (err: unknown) => {
          ...
          try {
            ...
            leao?.log?.error('[Data transfer] Failed to close socket on error');
          }
        };
        ...
        ws.on('close', async (...args) => {
          try {
            ...
            leao?.log?.error('[Data transfer] Uncaught error closing connection');
            leao?.log?.error(err);
        ...
        ws.on('error', async (...args) => {
          try {
            ...
            leao?.log?.error('[Data transfer] Uncaught error in error handling');
            leao?.log?.error(err);
            cannotRespondHandler(err);
          }
        });
        ws.on('message', async (...args) => {
          try {
            ...
            leao?.log?.error('[Data transfer] Uncaught error in message handling');
            leao?.log?.error(err);
            cannotRespondHandler(err);
          }
        });
      ...
      try {
        ...
        leao?.log?.error('[Data transfer] Error in websocket upgrade request');
        leao?.log?.error(err);
      }
    };
  };
