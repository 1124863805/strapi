...
const setPathIdAndPath = async (folder: Pick<Folder, 'parent'>) => {
  ...
  let parentPath = '/';
  if (folder.parent) {
    const parentFolder = await leao.db
...
const create = async (
  ...
  const folder = await leao.db.query(FOLDER_MODEL_UID).create({ data: enrichedFolder });

  leao.eventHub.emit('media-folder.create', { folder });
...
const deleteByIds = async (ids = []) => {
  const folders = await leao.db.query(FOLDER_MODEL_UID).findMany({ where: { id: { $in: ids } } });
  if (folders.length === 0) {
    return {
      folders: [],
      totalFolderNumber: 0,
  ...
  // delete files
  const filesToDelete = await leao.db.query(FILE_MODEL_UID).findMany({
    where: {
  ...
  const { count: totalFolderNumber } = await leao.db.query(FOLDER_MODEL_UID).deleteMany({
    where: {
      $or: pathsToDelete.flatMap((path) => [
        { path: { $eq: path } },
  ...
  leao.eventHub.emit('media-folder.delete', { folders });
...
const update = async (
  ...
  if (isUndefined(parent)) {
    ...
    const trx = await leao.db.transaction();
    try {
      // fetch existing folder
      const existingFolder = await leao.db
        .queryBuilder(FOLDER_MODEL_UID)
        .select(['pathId', 'path'])
        .where({ id })
        .transacting(trx.get())
      ...
      const { joinTable } = leao.db.metadata.get(FOLDER_MODEL_UID).attributes.parent;
      await leao.db
      ...
      if (parent !== null) {
        const destinationFolder = await leao.db
          .queryBuilder(FOLDER_MODEL_UID)
          .select('path')
          .where({ id: parent })
          .transacting(trx.get())
      ...
      const folderTable = leao.getModel(FOLDER_MODEL_UID).collectionName;
      const fileTable = leao.getModel(FILE_MODEL_UID).collectionName;
      const folderPathColumnName =
        // @ts-expect-error - no dynamic types
        leao.db.metadata.get(FILE_MODEL_UID).attributes.folderPath.columnName;
      // @ts-expect-error - no dynamic types
      const pathColumnName = leao.db.metadata.get(FOLDER_MODEL_UID).attributes.path.columnName;
      ...
      await leao.db
        ...
        .update(
          pathColumnName,
          leao.db.connection.raw('REPLACE(??, ?, ?)', [
            pathColumnName,
            existingFolder.path,
    ...
    leao.eventHub.emit('media-folder.update', { folder });
    return folder;
  }
};
...
const exists = async (params = {}) => {
  const count = await leao.db.query(FOLDER_MODEL_UID).count({ where: params });
  return count > 0;
};
...
const getStructure = async () => {
  // @ts-expect-error - no dynamic types
  const { joinTable } = leao.db.metadata.get(FOLDER_MODEL_UID).attributes.parent;
  const qb = leao.db.queryBuilder(FOLDER_MODEL_UID);
