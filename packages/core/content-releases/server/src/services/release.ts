...
const createReleaseService = ({ leao }: { leao: Core.Leao }) => {
  ...
  const getFormattedActions = async (releaseId: Release['id']) => {
    const actions = (await leao.db.query(RELEASE_ACTION_MODEL_UID).findMany({
      where: {
        release: {
          id: releaseId,
        },
  ...
  return {
    async create(releaseData: CreateRelease.Request['body'], { user }: { user: UserInfo }) {
      ...
      } = getService('release-validation', { leao });
      ...
      const release = await leao.db.query(RELEASE_MODEL_UID).create({
        data: {
    ...
    async findOne(id: GetRelease.Request['params']['id'], query = {}) {
      const dbQuery = leao.get('query-params').transform(RELEASE_MODEL_UID, query);
    ...
    findPage(query?: GetReleases.Request['query']) {
      ...
      return leao.db.query(RELEASE_MODEL_UID).findPage({
        ...dbQuery,
        populate: {
          actions: {
            count: true,
    ...
    findMany(query?: any) {
      ...
      return leao.db.query(RELEASE_MODEL_UID).findMany({
        ...dbQuery,
      });
    },
    ...
    async update(
      ...
      const release = await leao.db.query(RELEASE_MODEL_UID).findOne({ where: { id } });
      ...
      const updatedRelease = await leao.db.query(RELEASE_MODEL_UID).update({
        where: { id },
    ...
    async getAllComponents() {
      const contentManagerComponentsService = leao
    ...
    async delete(releaseId: DeleteRelease.Request['params']['id']) {
      const release: Release = await leao.db.query(RELEASE_MODEL_UID).findOne({
        where: { id: releaseId },
        populate: {
          actions: {
            select: ['id'],
      ...
      await leao.db.transaction(async () => {
        ...
        await leao.db.query(RELEASE_MODEL_UID).delete({
          where: {
            id: releaseId,
          },
        });
    ...
    async publish(releaseId: PublishRelease.Request['params']['id']) {
      const {
        ...
        await leao.db.transaction(async ({ trx }) => {
          /**
           ...
           */
          const lockedRelease = (await leao.db
          ...
          try {
            leao.log.info(`[Content Releases] Starting to publish release ${lockedRelease.name}`);
            ...
            await leao.db.transaction(async () =>
              Promise.all(
                Object.keys(formattedActions).map(async (contentTypeUid) => {
                  ...
                  return Promise.all([
                    ...publish.map((params) => leao.documents(contentType).publish(params)),
                    ...unpublish.map((params) => leao.documents(contentType).unpublish(params)),
                  ]);
                })
              )
            );

            const release = await leao.db.query(RELEASE_MODEL_UID).update({
              where: {
                id: releaseId,
              },
              data: {
    ...
    async updateReleaseStatus(releaseId: Release['id']) {
      ...
      if (totalActions > 0) {
        if (invalidActions > 0) {
          return leao.db.query(RELEASE_MODEL_UID).update({
            where: {
              id: releaseId,
            },
            data: {
