...
const develop = async ({
  ...
  if (cluster.isWorker) {
    timer.start('loadLeao');
    const loadLeaoSpinner = logger.spinner(`Loading Leao`).start();

    const leao = createLeao({
      appDir: cwd,
    ...
    if (tsconfig?.config || leao.config.get('typescript.autogenerate') !== false) {
    ...
    const restart = async () => {
      if (leaoInstance.reload.isWatching && !leaoInstance.reload.isReloading) {
        leaoInstance.reload.isReloading = true;
        leaoInstance.reload();
      }
    };
    ...
    const watcher = chokidar
      .watch(cwd, {
        ...
        ignored: [
          ...
          '**/public',
          '**/public/**',
          leaoInstance.dirs.static.public,
          strings.joinBy('/', leaoInstance.dirs.static.public, '**'),
          '**/*.db*',
          '**/exports/**',
          '**/dist/**',
          '**/*.d.ts',
          ...
          ...leaoInstance.config.get('admin.watchIgnoreFiles', []),
        ],
      })
      .on('add', (path) => {
        leaoInstance.log.info(`File created: ${path}`);
        restart();
      })
      .on('change', (path) => {
        leaoInstance.log.info(`File changed: ${path}`);
        restart();
      })
      .on('unlink', (path) => {
        leaoInstance.log.info(`File deleted: ${path}`);
        restart();
      });
    ...
    process.on('message', async (message) => {
      switch (message) {
        case 'kill': {
          logger.debug(
            'child process has the kill message, destroying the leao instance and sending the killed process message'
          );
          await watcher.close();
