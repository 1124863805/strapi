...
const list = async (): Promise<SanitizedTransferToken[]> => {
  const tokens: DatabaseTransferToken[] = await leao.db.query(TRANSFER_TOKEN_UID).findMany({
...
const create = async (attributes: TokenCreatePayload): Promise<TransferToken> => {
  ...
  const result = (await leao.db.transaction(async () => {
    const transferToken = await leao.db.query(TRANSFER_TOKEN_UID).create({
      select: SELECT_FIELDS,
      populate: POPULATE_FIELDS,
      data: {
    ...
    const currentPermissions: TransferTokenPermission[] = await leao.db
...
const update = async (
  ...
  const originalToken = await leao.db.query(TRANSFER_TOKEN_UID).findOne({ where: { id } });
  ...
  return leao.db.transaction(async () => {
    const updatedToken = await leao.db.query(TRANSFER_TOKEN_UID).update({
      select: SELECT_FIELDS,
      where: { id },
      data: {
        ...omit('permissions', attributes),
    ...
    if (attributes.permissions) {
      ...
      await Promise.all(
        actionsToDelete.map((action) =>
          leao.db.query(TRANSFER_TOKEN_PERMISSION_UID).delete({
      ...
      await Promise.all(
        actionsToAdd.map((action) =>
          leao.db.query(TRANSFER_TOKEN_PERMISSION_UID).create({
    ...
    const permissionsFromDb: TransferTokenPermission[] = await leao.db
      .query(TRANSFER_TOKEN_UID)
      .load(updatedToken, 'permissions');
...
const regenerate = async (id: string | number): Promise<TransferToken> => {
  ...
  const transferToken = (await leao.db.transaction(async () =>
    leao.db.query(TRANSFER_TOKEN_UID).update({
      select: ['id', 'accessKey'],
      where: { id },
      data: {
        accessKey: hash(accessKey),
...
const hash = (accessKey: string): string => {
  ...
  return crypto
    .createHmac('sha512', leao.config.get('admin.transfer.token.salt'))
    .update(accessKey)
    .digest('hex');
};
...
const checkSaltIsDefined = () => {
  ...
  if (!leao.config.get('server.transfer.remote.enabled')) {
    return;
  }
...
const assertTokenPermissionsValidity = (attributes: TokenUpdatePayload) => {
  const permissionService = leao.service('admin::transfer').permission;
