...
const getFolderPath = async (folderId?: number | null) => {
  ...
  const parentFolder = await leao.db.query(FOLDER_MODEL_UID).findOne({ where: { id: folderId } });

  return parentFolder.path;
};

const deleteByIds = async (ids: number[] = []) => {
  const filesToDelete = await leao.db
...
const signFileUrls = async (file: File) => {
  ...
  const { provider: providerConfig } = leao.config.get<Config>('plugin::upload');
