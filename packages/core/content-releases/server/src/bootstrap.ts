...
export const bootstrap = async ({ leao }: { leao: Core.Leao }) => {
  if (leao.ee.features.isEnabled('cms-content-releases')) {
    const contentTypesWithDraftAndPublish = Object.keys(leao.contentTypes).filter(
      (uid: any) => leao.contentTypes[uid]?.options?.draftAndPublish
    );
    ...
    leao.db.lifecycles.subscribe({
      ...
      async afterDeleteMany(event) {
        try {
          const model = leao.getModel(event.model.uid as UID.Schema);
          ...
          leao.log.error('Error while deleting release actions after entry deleteMany', {
            error,
          });
        }
      },
    ...
    leao.documents.use(deleteActionsOnDelete);
    leao.documents.use(updateActionsOnUpdate);

    getService('scheduling', { leao })
      .syncFromDatabase()
      .catch((err: Error) => {
        leao.log.error(
    ...
    Object.entries(ALLOWED_WEBHOOK_EVENTS).forEach(([key, value]) => {
      leao.get('webhookStore').addAllowedEvent(key, value);
    });
  }
};
