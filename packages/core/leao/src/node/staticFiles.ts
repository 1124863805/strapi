...
const writeStaticClientFiles = async (ctx: BuildContext) => {
  ...
  /**
   * For everything to work effectively we create a client folder in `.leao` at the cwd level.
  ...
  await fs.writeFile(path.join(ctx.runtimeDir, 'tailwind.css'), TAILWIND_CSS);
  ctx.logger.debug('Wrote the tailwind.css file');
};
