...
export const deleteRecords = async (leao: Core.Leao, options: IRestoreOptions) => {
  const entities = await deleteEntitiesRecords(leao, options);
  const configuration = await deleteConfigurationRecords(leao, options);
...
const deleteEntitiesRecords = async (
  ...
  const models = leao.get('models').get() as Model[];
  const contentTypes = Object.values(leao.contentTypes) as Struct.ContentTypeSchema[];
  ...
  const contentTypeQuery = queries.entity.createEntityQuery(leao);
  ...
  const modelsPromises = modelsToClear.map(async (uid) => {
    const result = await leao.db.query(uid).deleteMany({});
...
const deleteConfigurationRecords = async (
  ...
  if (coreStore) {
    models.push('leao::core-store');
  }

  if (webhook) {
    models.push('leao::webhook');
  }
