...
const shouldCreateHistoryVersion = (
  ...
  if (!leao.requestContext.get()?.request.url.startsWith('/content-manager')) {
    return false;
  }
  ...
  if (
    context.action === 'update' &&
    leao.requestContext.get()?.request.url.endsWith('/actions/publish')
  ) {
    return false;
  }
...
const getSchemas = (uid: UID.CollectionType) => {
  const attributesSchema = leao.getModel(uid).attributes;
  ...
  const componentsSchemas = Object.keys(attributesSchema).reduce(
    (currentComponentSchemas, key) => {
      ...
      if (fieldSchema.type === 'component') {
        const componentSchema = leao.getModel(fieldSchema.component).attributes;
        return {
...
const createLifecyclesService = ({ leao }: { leao: Core.Leao }) => {
  const state: {
  ...
  return {
    async bootstrap() {
      ...
      leao.documents.use(async (context, next) => {
        ...
        const localeEntries = await leao.db.query(uid).findMany({
          where: {
            documentId,
            ...(isLocalizedContentType ? { locale: { $in: locales } } : {}),
            ...(contentTypes.hasDraftAndPublish(leao.contentTypes[uid])
              ? { publishedAt: null }
              : {}),
          },
        ...
        await leao.db.transaction(async ({ onCommit }) => {
          ...
          onCommit(async () => {
            for (const entry of localeEntries) {
              ...
              await getService(leao, 'history').createVersion({
                contentType: uid,
        ...
        leao.db
          ...
          .catch((error) => {
            if (error instanceof Error) {
              leao.log.error('Error deleting expired history versions', error.message);
            }
          });
      });
