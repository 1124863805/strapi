...
const sanitizeOutput = async (user, ctx) => {
  const schema = leao.getModel('plugin::users-permissions.user');
  const { auth } = ctx.state;

  return leao.contentAPI.sanitize.output(user, schema, { auth });
};
...
const validateQuery = async (query, ctx) => {
  ...
  return leao.contentAPI.validate.query(query, schema, { auth });
};
...
const sanitizeQuery = async (query, ctx) => {
  ...
  return leao.contentAPI.sanitize.query(query, schema, { auth });
};
...
module.exports = {
  ...
  async create(ctx) {
    ...
    if (!role) {
      const defaultRole = await leao.db
