...
export const createEntitiesWriteStream = (options: IEntitiesRestoreStreamOptions) => {
  const { leao, updateMappingTable, transaction } = options;
  const query = queries.entity.createEntityQuery(leao);
  ...
  return new Writable({
    ...
    async write(entity: IEntity, _encoding, callback) {
      await transaction?.attach(async () => {
        ...
        const contentType = leao.getModel(type);
        ...
        try {
          ...
          diffs.forEach((diff) => {
            if (diff.kind === 'modified' && last(diff.path) === 'id' && 'kind' in contentType) {
              const target = resolveComponentUID({ paths: diff.path, data, contentType, leao });
