...
const find = (params: any = {}) =>
  leao.db.query('plugin::i18n.locale').findMany({ where: params });

const findById = (id: any) => leao.db.query('plugin::i18n.locale').findOne({ where: { id } });

const findByCode = (code: any) =>
  leao.db.query('plugin::i18n.locale').findOne({ where: { code } });
...
const initDefaultLocale = async () => {
  const existingLocalesNb = await leao.db.query('plugin::i18n.locale').count();
...
const deleteAllLocalizedEntriesFor = async ({ locale }: any) => {
  ...
  const localizedModels = Object.values(leao.contentTypes).filter(isLocalizedContentType);

  for (const model of localizedModels) {
    // FIXME: delete many content & their associations
    await leao.db.query(model.uid).deleteMany({ where: { locale } });
  }
};
