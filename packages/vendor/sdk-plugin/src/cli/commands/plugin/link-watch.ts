  ...
  try {
    ...
    logger.info(
      outdent`
          ...
          To use this package in Leao, in a separate shell run:
          cd /path/to/leao/project
...
/**
 * `$ leao-plugin watch:link`
 */
const command: LeaoCommand = ({ command: commanderCommand, ctx }) => {
  commanderCommand
    .command('watch:link')
