...
export const createLinkQuery = (leao: Core.Leao, trx?: Knex.Transaction) => {
  const query = () => {
    const { connection } = leao.db;
    ...
    async function* generateAllForAttribute(uid: string, fieldName: string): AsyncGenerator<ILink> {
      const metadata = leao.db.metadata.get(uid);
    ...
    const insert = async (link: ILink) => {
      ...
      const metadata = leao.db.metadata.get(left.type);
