...
import { createLocalLeaoDestinationProvider } from '../../providers';
...
export interface PushHandler extends Handler {
  /**
   * Local Leao Destination Provider used to write data to the current Leao instance
   */
  provider?: ReturnType<typeof createLocalLeaoDestinationProvider>;
...
export const createPushController = handlerControllerFactory<Partial<PushHandler>>((proto) => ({
  ...
  async init(
    ...
    this.provider = createLocalLeaoDestinationProvider({
      ...params.options,
      autoDestroy: false,
      getLeao: () => leao as Core.Leao,
    });

    this.provider.onWarning = (message) => {
      // TODO send a warning message to the client
      leao.log.warn(message);
    };
