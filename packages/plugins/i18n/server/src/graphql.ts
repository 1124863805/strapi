...
export default ({ leao }: { leao: Core.Leao }) => ({
  register() {
    ...
    const { service: getI18NService } = leao.plugin('i18n');
    ...
    Object.entries(leao.contentTypes).forEach(([uid, ct]) => {
...
const getI18nLocaleArgPlugin = ({ nexus, typeRegistry }: any) => {
  ...
  return nexus.plugin({
    ...
    onAddOutputField(config: any) {
      ...
      if (config?.extensions?.leao?.contentType) {
        contentType = config.extensions.leao.contentType;
      } else {
