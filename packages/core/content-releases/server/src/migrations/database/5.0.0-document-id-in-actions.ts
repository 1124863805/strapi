...
export const addEntryDocumentToReleaseActions: Migration = {
  ...
  async up(trx: Knex, db: Database) {
    ...
    const hasTable = await trx.schema.hasTable('leao_release_actions');

    if (!hasTable) {
      return;
    }

    const hasPolymorphicColumn = await trx.schema.hasColumn('leao_release_actions', 'target_id');
    ...
    if (hasPolymorphicColumn) {
      ...
      const hasEntryDocumentIdColumn = await trx.schema.hasColumn(
        'leao_release_actions',
        'entry_document_id'
      );

      if (!hasEntryDocumentIdColumn) {
        await trx.schema.alterTable('leao_release_actions', (table) => {
          table.string('entry_document_id');
        });
      }

      const releaseActions = await trx.select('*').from('leao_release_actions');
      ...
      async.map(releaseActions, async (action: any) => {
        ...
        if (entry) {
          await trx('leao_release_actions')
