...
module.exports = ({ leao }) => ({
  getActions({ defaultEnable = false } = {}) {
    ...
    _.forEach(leao.apis, (api, apiName) => {
    ...
    _.forEach(leao.plugins, (plugin, pluginName) => {
      const controllers = _.reduce(
        plugin.controllers,
        (acc, controller, controllerName) => {
          const contentApiActions = _.pickBy(controller, isContentApi);
  ...
  async syncPermissions() {
    const roles = await leao.db.query('plugin::users-permissions.role').findMany();
    const dbPermissions = await leao.db.query('plugin::users-permissions.permission').findMany();

    const permissionsFoundInDB = _.uniq(_.map(dbPermissions, 'action'));

    const appActions = _.flatMap(leao.apis, (api, apiName) => {
    ...
    const pluginsActions = _.flatMap(leao.plugins, (plugin, pluginName) => {
    ...
    if (permissionsFoundInDB.length === 0) {
      ...
      for (const role of roles) {
        ...
        await Promise.all(
          toCreate.map((action) => {
            return leao.db.query('plugin::users-permissions.permission').create({
              data: {
                action,
                role: role.id,
              },
  ...
  async initialize() {
    const roleCount = await leao.db.query('plugin::users-permissions.role').count();

    if (roleCount === 0) {
      await leao.db.query('plugin::users-permissions.role').create({
        data: {
          name: 'Authenticated',
