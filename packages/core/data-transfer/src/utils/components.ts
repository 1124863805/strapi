...
const isDialectMySQL = () => leao.db?.dialect.client === 'mysql';
    ...
    if (attribute.type === 'component') {
      ...
      if (repeatable === true) {
        ...
        const components = (await async.map(
          ...
          { concurrency: isDialectMySQL() && !leao.db?.inTransaction() ? 1 : Infinity }
...
const getComponents = async <TUID extends UID.Schema>(
  ...
  const componentAttributes = contentTypesUtils.getComponentAttributes(leao.getModel(uid));

  if (_.isEmpty(componentAttributes)) {
    return {} as LoadedComponents<TUID>;
  }

  return leao.db.query(uid).load(entity, componentAttributes) as Promise<LoadedComponents<TUID>>;
};
...
const deleteOldComponents = async <TUID extends UID.Schema>(
  ...
  const previousValue = (await leao.db
    .query(uid)
...
const deleteComponents = async <TUID extends UID.Schema, TEntity extends Data.Entity<TUID>>(
  ...
  for (const attributeName of attributeNames) {
    ...
    if (attribute.type === 'component' || attribute.type === 'dynamiczone') {
      let value;
      if (loadComponents) {
        value = await leao.db.query(uid).load(entityToDelete, attributeName);
      } else {
...
const createComponent = async <TUID extends UID.Component = UID.Component>(
  ...
  const model = leao.getModel(uid) as Schema.Component;
  ...
  return leao.db.query(uid).create({ data: transform(data) });
};
...
const updateComponent = async <TUID extends UID.Component>(
  ...
  return leao.db.query(uid).update({
    where: {
...
const deleteComponent = async <TUID extends UID.Component>(
  ...
  await leao.db.query(uid).delete({ where: { id: componentToDelete.id } });
};
...
const resolveComponentUID = ({
  ...
  for (const path of paths) {
    ...
    if (path in cType.attributes) {
      ...
      if (attribute.type === 'component') {
        cType = leao.getModel(attribute.component);
      }

      if (attribute.type === 'dynamiczone') {
        cType = ({ __component }: { __component: UID.Component }) => leao.getModel(__component);
      }
    }
  }
