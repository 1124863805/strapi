...
module.exports = ({ leao }) => ({
  ...
  async find() {
    const roles = await leao.db
      .query('plugin::users-permissions.role')
      .findMany({ sort: ['name'] });

    for (const role of roles) {
      role.nb_users = await leao.db
  ...
  async updateRole(roleID, data) {
    ...
    await leao.db.query('plugin::users-permissions.role').update({
      where: { id: roleID },
    ...
    await Promise.all(
      toCreate.map((permissionInfo) =>
        leao.db.query('plugin::users-permissions.permission').create({ data: permissionInfo })
      )
    );
  },
  ...
  async deleteRole(roleID, publicRoleID) {
    ...
    await Promise.all(
      role.users.map((user) => {
        return leao.db.query('plugin::users-permissions.user').update({
          where: { id: user.id },
    ...
    await Promise.all(
      role.permissions.map((permission) => {
        return leao.db.query('plugin::users-permissions.permission').delete({
    ...
    // Delete the role.
    await leao.db.query('plugin::users-permissions.role').delete({ where: { id: roleID } });
  },
});
