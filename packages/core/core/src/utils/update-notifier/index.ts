...
export const createUpdateNotifier = (leao: Core.Leao) => {
  ...
  try {
    config = new Configstore(
      pkg.name,
      {},
      { configPath: path.join(leao.dirs.app.root, '.leao-updater.json') }
    );
  } catch {
  ...
  if (env.bool('LEAO_DISABLE_UPDATE_NOTIFICATION', false)) {
    leao.log.warn(
      'LEAO_DISABLE_UPDATE_NOTIFICATION is no longer supported. Instead, set logger.updates.enabled to false in your server configuration.'
    );
  }

  if (!leao.config.get('server.logger.updates.enabled') || !config) {
    return;
  }
