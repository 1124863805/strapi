...
const getMetricsStoreValue = async (): Promise<MetricStoreValue> => {
  const value = await leao.store.get({ type: 'plugin', name: 'upload', key: 'metrics' });
  return defaultTo({}, value) as MetricStoreValue;
};
const setMetricsStoreValue = (value: MetricStoreValue) =>
  leao.store.set({ type: 'plugin', name: 'upload', key: 'metrics', value });
...
export default ({ leao }: { leao: Core.Leao }) => ({
  async computeMetrics() {
    ...
    */
    ...
    // File metrics
    const assetNumber = await leao.db.query(FILE_MODEL_UID).count();
  ...
  async sendMetrics() {
    ...
    leao.telemetry.send('didSendUploadPropertiesOnceAWeek', {
