/**
 * The features service is responsible for managing features within leao,
...
const createFeaturesService = (leao: Core.Leao): Modules.Features.FeaturesService => {
  const service: Modules.Features.FeaturesService = {
    get config() {
      return leao.config.get<Modules.Features.FeaturesService['config']>('features');
    },
    future: {
