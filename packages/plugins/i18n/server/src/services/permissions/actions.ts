...
const shouldApplyLocalesPropertyToSubject = ({ property, subject }: any) => {
  if (property === 'locales') {
    const model = leao.getModel(subject);
...
const syncSuperAdminPermissionsWithLocales = async () => {
  const roleService = leao.service('admin::role');
  const permissionService = leao.service('admin::permission');
...
const registerI18nActionsHooks = () => {
  const { actionProvider } = leao.service('admin::permission');
  const { hooks } = leao.service('admin::role');
