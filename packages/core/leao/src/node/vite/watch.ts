...
const watch = async (ctx: BuildContext): Promise<ViteWatcher> => {
  ...
  if (typeof hmrConfig === 'object' && hmrConfig.server === hmrServer) {
    // Only restart the hmr server when Leao's server is listening
    leao.server.httpServer.on('listening', async () => {
  ...
  ctx.leao.server.app.use((ctx, next) => {
  ...
  const serveAdmin: Core.MiddlewareHandler = async (koaCtx, next) => {
    ...
    let template = await fs.readFile(path.relative(ctx.cwd, '.leao/client/index.html'), 'utf-8');
