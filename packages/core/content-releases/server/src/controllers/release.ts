...
const releaseController = {
  ...
  async findByDocumentAttached(ctx: Koa.Context) {
    ...
    const model = leao.getModel(query.contentType) as Schema.ContentType;
  ...
  async findPage(ctx: Koa.Context) {
    ...
    const pendingReleasesCount = await leao.db.query(RELEASE_MODEL_UID).count({
      where: {
        releasedAt: null,
      },
    });
  ...
  async findOne(ctx: Koa.Context) {
    ...
    const sanitizedRelease = {
      ...
      createdBy: release.createdBy
        ? leao.service('admin::user').sanitizeUser(release.createdBy)
        : null,
    };
