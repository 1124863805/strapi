...
/**
 * Helper functions for the Leao CLI
 */
...
const assertCwdContainsLeaoProject = (name: string) => {
  const logErrorAndExit = () => {
    console.log(
      `You need to run ${chalk.yellow(
        `leao ${name}`
      )} in a Leao project. Make sure you are in the right directory.`
    );
    process.exit(1);
  };
...
const runAction =
  ...
  (...args: unknown[]) => {
    assertCwdContainsLeaoProject(name);
