...
import type { Core, Struct, UID } from '@leao1/types';
...
export default async function loadComponents(leao: Core.Leao) {
  if (!(await pathExists(leao.dirs.dist.components))) {
    return {};
  }

  const map = await loadFiles<LoadedComponents>(leao.dirs.dist.components, '*/*.*(js|json)');
  ...
  const components = Object.keys(map).reduce((acc, category) => {
    Object.keys(map[category]).forEach((key) => {
      ...
      if (!schema.collectionName) {
        ...
        const filePath = join(leao.dirs.app.components, category, schema.__filename__);

        return leao.stopWithError(
  ...
  leao.get('components').add(components);
}
