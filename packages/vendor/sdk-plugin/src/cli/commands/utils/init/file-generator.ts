...
export const generateFiles = async (
  ...
  const pkgJson: PluginPackageJson = {
    ...
    files: ['dist'],
    scripts: {
      build: 'leao-plugin build',
      watch: 'leao-plugin watch',
      'watch:link': 'leao-plugin watch:link',
      verify: 'leao-plugin verify',
    },
    dependencies: {},
    devDependencies: {
      '@leao1/leao': '*',
  ...
  for (const ans of answers) {
    ...
    switch (name) {
      ...
      case 'pluginId': {
        pkgJson.leao.name = String(answer);
        break;
      }
      case 'description': {
        pkgJson.description = String(answer);
        pkgJson.leao.description = String(answer);
        break;
      }
      case 'displayName': {
        pkgJson.leao.displayName = String(answer);
        break;
      }
      case 'authorName': {
      ...
      case 'typescript': {
        ...
        if (isTypescript) {
          if (isRecord(pkgJson.exports['./leao-admin'])) {
            pkgJson.exports['./leao-admin'].source = './admin/src/index.ts';
          ...
          if (isRecord(pkgJson.exports['./leao-server'])) {
            pkgJson.exports['./leao-server'].source = './server/src/index.ts';
  ...
  // Ensure leao.name is always set (plugin identifier used by Leao)
  if (!pkgJson.leao.name) {
    pkgJson.leao.name = pluginId;
  }
