...
async function signFile(file: IFile) {
  const { provider } = leao.plugins.upload;
  const { provider: providerName } = leao.config.get('plugin.upload') as { provider: string };
 ...
 */
export const createAssetsStream = (leao: Core.Leao): Duplex => {
  const generator: () => AsyncGenerator<IAsset, void> = async function* () {
    const stream: Readable = leao.db
    ...
    for await (const file of stream) {
      ...
      const filepath = isLocalProvider ? join(leao.dirs.static.public, file.url) : file.url;
      const stats = await getFileStats(filepath, leao, isLocalProvider);
      const stream = getFileStream(filepath, leao, isLocalProvider);
      ...
      if (file.formats) {
        for (const format of Object.keys(file.formats)) {
          ...
          const fileFormatFilepath = isLocalProvider
            ? join(leao.dirs.static.public, fileFormat.url)
            : fileFormat.url;
          const fileFormatStats = await getFileStats(fileFormatFilepath, leao, isLocalProvider);
          const fileFormatStream = getFileStream(fileFormatFilepath, leao, isLocalProvider);
