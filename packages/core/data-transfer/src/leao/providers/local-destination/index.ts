...
export interface ILocalLeaoDestinationProviderOptions {
  ...
  restore?: restore.IRestoreOptions; // erase data in leao database before transfer; required if strategy is 'restore'
  strategy: 'restore'; // conflict management strategy; only the restore strategy is available at this time
}

class LocalLeaoDestinationProvider implements IDestinationProvider {
  name = 'destination::local-leao';

  type: ProviderType = 'destination';

  options: ILocalLeaoDestinationProviderOptions;
  ...
  constructor(options: ILocalLeaoDestinationProviderOptions) {
  ...
  async bootstrap(): Promise<void> {
    ...
    if (!this.leao) {
      throw new ProviderInitializationError('Could not access local leao');
    }

    this.transaction = utils.transaction.createTransaction(this.leao);
  }
  ...
  async #deleteFromRestoreOptions() {
    assertValidLeao(this.leao);
    if (!this.options.restore) {
      throw new ProviderValidationError('Missing restore options');
    }
    return restore.deleteRecords(this.leao, this.options.restore);
  }
  ...
  async #deleteAllAssets(trx?: Knex.Transaction) {
    ...
    const stream: Readable = this.leao.db
    ...
    for await (const file of stream) {
      await this.leao.plugin('upload').provider.delete(file);
      if (file.formats) {
        for (const fileFormat of Object.values(file.formats)) {
          await this.leao.plugin('upload').provider.delete(fileFormat);
        }
      }
    }
  }
  ...
  async beforeTransfer() {
    if (!this.leao) {
      throw new Error('Leao instance not found');
    }
  ...
  getMetadata(): IMetadata {
    ...
    const leaoVersion = this.leao.config.get<string>('info.leao');
  ...
  createEntitiesWriteStream(): Writable {
    assertValidLeao(this.leao, 'Not able to import entities');
  ...
  async #handleAssetsBackup() {
    assertValidLeao(this.leao, 'Not able to create the assets backup');
    ...
    if (this.leao.config.get<{ provider: string }>('plugin::upload').provider === 'local') {
      const assetsDirectory = path.join(this.leao.dirs.static.public, 'uploads');
      const backupDirectory = path.join(
        this.leao.dirs.static.public,
      ...
      try {
        ...
        throw new ProviderTransferError(
          'The backup folder for the assets could not be created inside the public folder. Please ensure Leao has write permissions on the public directory',
          {
  ...
  async #removeAssetsBackup() {
    assertValidLeao(this.leao, 'Not able to remove Assets');
    // if we're not restoring assets, don't back them up because they won't be touched
    if (!this.#areAssetsIncluded()) {
      return;
    }
      ...
      async write(chunk: IAsset, _encoding, callback) {
        await transaction?.attach(async () => {
          ...
          if (!chunk.metadata) {
            ...
            const assetsDirectory = path.join(leao.dirs.static.public, 'uploads');
          ...
          const provider = leao.config.get<{ provider: string }>('plugin::upload').provider;

          try {
            await leao.plugin('upload').provider.uploadStream(uploadData);
            ...
            if (uploadData?.type) {
              ...
              const entry: IFile = await leao.db.query('plugin::upload.file').findOne({
                where: condition,
              });
              ...
              await leao.db.query('plugin::upload.file').update({
                where: { id: entry.id },
                data: {
  ...
  async createConfigurationWriteStream(): Promise<Writable> {
    assertValidLeao(this.leao, 'Not able to stream Configurations');

    const { strategy } = this.options;

    if (strategy === 'restore') {
      return restore.createConfigurationWriteStream(this.leao, this.transaction);
    }
  ...
  async createLinksWriteStream(): Promise<Writable> {
    if (!this.leao) {
      throw new Error('Not able to stream links. Leao instance not found');
    }
    ...
    if (strategy === 'restore') {
      return restore.createLinksWriteStream(mapID, this.leao, this.transaction, this.onWarning);
    }
...
export const createLocalLeaoDestinationProvider = (
  options: ILocalLeaoDestinationProviderOptions
) => {
  return new LocalLeaoDestinationProvider(options);
};
