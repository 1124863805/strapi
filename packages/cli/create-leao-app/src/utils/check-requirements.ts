...
export function checkNodeRequirements() {
  ...
  if (!semver.satisfies(currentNodeVersion, engines.node)) {
    logger.fatal([
      ...
      `Leao requires ${chalk.bold(chalk.green(`Node.js ${engines.node}`))}`,
  ...
  // warn if not using a LTS version
    ...
    logger.warn([
      ...
      `Leao only supports ${chalk.bold(chalk.green('LTS versions of Node.js'))}, other versions may not be compatible.`,
    ]);
  }
}
