 ...
 */
export const createEntitiesStream = (leao: Core.Leao): Readable => {
  const contentTypes: Struct.ContentTypeSchema[] = Object.values(leao.contentTypes);

  async function* contentTypeStreamGenerator() {
    for (const contentType of contentTypes) {
      const query = shared.entity.createEntityQuery(leao).call(null, contentType.uid);
