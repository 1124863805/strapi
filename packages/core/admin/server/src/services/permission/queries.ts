...
export const deleteByRolesIds = async (rolesIds: Data.ID[]): Promise<void> => {
  const permissionsToDelete = await leao.db.query('admin::permission').findMany({
    select: ['id'],
    where: {
      role: { id: rolesIds },
    },
...
export const deleteByIds = async (ids: Data.ID[]): Promise<void> => {
  ...
  for (const id of ids) {
    const queryResult = await leao.db.query('admin::permission').delete({ where: { id } });
    result.push(queryResult);
  }
  leao.eventHub.emit('permission.delete', { permissions: result });
};
...
export const createMany = async (permissions: CreatePermissionPayload[]): Promise<Permission[]> => {
  ...
  for (const permission of permissions) {
    const newPerm = await leao.db.query('admin::permission').create({ data: permission });
    createdPermissions.push(newPerm);
  }

  const permissionsToReturn = permissionDomain.toPermission(createdPermissions);
  leao.eventHub.emit('permission.create', { permissions: permissionsToReturn });
...
const update = async (params: unknown, attributes: Partial<Permission>) => {
  const updatedPermission = (await leao.db
  ...
  leao.eventHub.emit('permission.update', { permissions: permissionToReturn });
...
export const findMany = async (params = {}): Promise<Permission[]> => {
  const rawPermissions = await leao.db.query('admin::permission').findMany(params);
