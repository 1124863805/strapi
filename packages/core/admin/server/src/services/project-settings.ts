...
const parseFilesData = async (files: UpdateProjectSettings.Request['files']) => {
  ...
  await Promise.all(
    PROJECT_SETTINGS_FILE_INPUTS.map(async (inputName) => {
      ...
      formatedFilesData[inputName] = await leao
        .plugin('upload')
        .service('upload')
        .formatFileInfo({
      ...
      Object.assign(
        ...
        await leao.plugin('upload').service('image-manipulation').getDimensions({ getStream })
      );
      ...
      Object.assign(formatedFilesData[inputName]!, {
        ...
        provider: leao.config.get('plugin::upload').provider,
      });
    })
  );
...
const getProjectSettings = async (): Promise<GetProjectSettings.Response> => {
  const store = leao.store({ type: 'core', name: 'admin' });
...
const uploadFiles = async (files: LogoFiles = {}) => {
  ...
  return Promise.all(
    Object.values(files)
      ...
      .map((file) => leao.plugin('upload').provider.uploadStream(file))
  );
};
...
const deleteOldFiles = async ({ previousSettings, newSettings }: any) => {
  return Promise.all(
    PROJECT_SETTINGS_FILE_INPUTS.map(async (inputName) => {
      ...
      // TODO
      // @ts-expect-error define the correct return type
      if (leao.config.get('plugin::upload').provider !== previousSettings[inputName].provider) {
        return;
      }

      // There was a previous file and an new file was uploaded
      // Remove the previous file
      leao.plugin('upload').provider.delete(previousSettings[inputName]);
    })
  );
};
