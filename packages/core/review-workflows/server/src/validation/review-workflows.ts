...
const validateContentTypes = yup.array().of(
  yup
    ...
    .test({
      ...
      test(uid: any) {
        // Warning; we use the leao global - to avoid that, it would need to refactor how
        // we generate validation function by using a factory with the leao instance as parameter.
        return !!leao.getModel(uid);
      },
    })
    .test({
