...
export default ({ leao }: Context) => {
  ...
  const recursivelyReplaceScalarOperators = (data: any): any => {
    ...
    for (const [key, value] of Object.entries(data)) {
      ...
      const newKey = isOperator ? operators[key].leaoOperator : key;
  ...
  return {
    /**
     * Transform one or many GraphQL filters object into a valid Leao query
     * @param {object | object[]} filters
     * @param {object} contentType
     * @return {object | object[]}
     */
    graphQLFiltersToLeaoQuery(filters: any, contentType: Struct.Schema): any {
      const { isLeaoScalar, isMedia, isRelation, isComponent } = getService('utils').attributes;
      const { operators } = getService('builders').filters;
      ...
      if (Array.isArray(filters)) {
        return filters.map((filtersItem) =>
          this.graphQLFiltersToLeaoQuery(filtersItem, contentType)
        );
      }
      ...
      for (const [key, value] of Object.entries(filters)) {
        ...
        if (isAttribute(key)) {
          ...
          if (virtualScalarAttributes.includes(key) || isLeaoScalar(attribute)) {
            // Replace (recursively) every GraphQL scalar operator with the associated Leao operator
          ...
          // If it's a deep filter on a relation
            ...
            const relModel = leao.getModel(attribute.target);

            // Recursively apply the mapping to the value using the fetched model,
            // and update the value within `resultMap`
            resultMap[key] = this.graphQLFiltersToLeaoQuery(value, relModel);
          }

          // If it's a deep filter on a component
          else if (isComponent(attribute)) {
            // Fetch the model from the component attribute
            const componentModel = leao.getModel(attribute.component);

            // Recursively apply the mapping to the value using the fetched model,
            // and update the value within `resultMap`
            resultMap[key] = this.graphQLFiltersToLeaoQuery(value, componentModel);
          }
        }
        ...
        // Handle the case where the key is not an attribute (operator, ...)
          ...
          if (rootLevelOperator) {
            const { leaoOperator } = rootLevelOperator;

            // Transform the current value recursively and add it to the resultMap
            // object using the leaoOperator equivalent of the GraphQL key
            resultMap[leaoOperator] = this.graphQLFiltersToLeaoQuery(value, contentType);
          }
        }
      }
