...
const createReplaceImporter = (db: Database) => {
  ...
  return {
    ...
    async import(conf: Record<string, unknown>) {
      const matching = await db.query('leao::core-store').count({ where: { key: conf.key } });
      if (matching > 0) {
        stats.replaced += 1;
        await db.query('leao::core-store').update({
          where: { key: conf.key },
          data: conf,
        });
      } else {
        stats.created += 1;
        await db.query('leao::core-store').create({ data: conf });
      }
    },
  };
};
