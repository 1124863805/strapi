...
const create = async (
  ...
  const createdUser = await leao.db
    .query('admin::user')
...
const updateById = async (
  ...
  if (_.has(attributes, 'password')) {
    ...
    const updatedUser = await leao.db.query('admin::user').update({
      where: { id },
      data: {
        ...attributes,
    ...
    leao.eventHub.emit('user.update', { user: sanitizeUser(updatedUser) });
...
const exists = async (attributes = {} as unknown): Promise<boolean> => {
  return (await leao.db.query('admin::user').count({ where: attributes })) > 0;
};
...
const register = async ({
  ...
  const matchingUser = await leao.db
    .query('admin::user')
...
const findOne = async (id: Data.ID, populate = ['roles']) => {
  return leao.db.query('admin::user').findOne({ where: { id }, populate });
};
...
const findOneByEmail = async (email: string, populate = []) => {
  return leao.db.query('admin::user').findOne({
...
const findPage = async (params = {}): Promise<unknown> => {
  ...
  return leao.db.query('admin::user').findPage(query);
};
...
const deleteById = async (id: Data.ID): Promise<AdminUser | null> => {
  // Check at least one super admin remains
  const userToDelete: AdminUser | null = await leao.db.query('admin::user').findOne({
    where: { id },
    populate: ['roles'],
  });
  ...
  const deletedUser = await leao.db
    .query('admin::user')
    .delete({ where: { id }, populate: ['roles'] });

  leao.eventHub.emit('user.delete', { user: sanitizeUser(deletedUser) });
...
const deleteByIds = async (ids: (string | number)[]): Promise<AdminUser[]> => {
  // Check at least one super admin remains
  const superAdminRole = await getService('role').getSuperAdminWithUsersCount();
  const nbOfSuperAdminToDelete = await leao.db.query('admin::user').count({
    where: {
      id: ids,
  ...
  for (const id of ids) {
    const deletedUser = await leao.db.query('admin::user').delete({
      where: { id },
      populate: ['roles'],
    });
  ...
  leao.eventHub.emit('user.delete', {
...
const countUsersWithoutRole = async (): Promise<number> => {
  return leao.db.query('admin::user').count({
    where: {
      roles: {
        id: { $null: true },
      },
...
const count = async (where = {}): Promise<number> => {
  return leao.db.query('admin::user').count({ where });
};
...
const assignARoleToAll = async (roleId: Data.ID): Promise<void> => {
  const users = await leao.db.query('admin::user').findMany({
    select: ['id'],
    where: {
      roles: { id: { $null: true } },
    },
  ...
  await Promise.all(
    users.map((user) => {
      return leao.db.query('admin::user').update({
        where: { id: user.id },
        data: { roles: [roleId] },
      });
    })
...
const displayWarningIfUsersDontHaveRole = async (): Promise<void> => {
  ...
  if (count > 0) {
    leao.log.warn(`Some users (${count}) don't have any role.`);
  }
};
...
const getLanguagesInUse = async (): Promise<string[]> => {
  const users = await leao.db.query('admin::user').findMany({ select: ['preferedLanguage'] });
