...
export async function migrateIsValidAndStatusReleases() {
  const releasesWithoutStatus = (await leao.db.query(RELEASE_MODEL_UID).findMany({
    where: {
      status: null,
      releasedAt: null,
    },
  ...
  async.map(releasesWithoutStatus, async (release: Release) => {
    ...
    return getService('release', { leao }).updateReleaseStatus(release.id);
  });

  const publishedReleases = await leao.db.query(RELEASE_MODEL_UID).findMany({
    where: {
      status: null,
      releasedAt: {
        $notNull: true,
    ...
    async
      ...
      .then(() => {
        ...
        async.map(releasesAffected, async (releaseId: Release['id']) => {
          return getService('release', { leao }).updateReleaseStatus(releaseId);
        });
      });
  }
}
...
export async function disableContentTypeLocalized({ oldContentTypes, contentTypes }: Input) {
  ...
  const i18nPlugin = leao.plugin('i18n');
  if (!i18nPlugin) {
    return;
  }
