...
async function createLeao(scope: Scope) {
...
async function createApp(scope: Scope) {
  ...
  logger.title('Leao', `Creating a new application at ${chalk.green(rootPath)}`);
  ...
  logger.log([
    ...
    '',
    'Start Leao in watch mode. (Changes in Leao project files will trigger a server restart)',
    `${cmd} develop`,
    '',
    'Start Leao without watch mode.',
    `${cmd} start`,
    '',
    'Build Leao admin panel.',
    `${cmd} build`,
    '',
    'Deploy Leao project.',
    `${cmd} deploy`,
  ...
  if (runApp && installDependencies) {
    logger.title('Run', 'Running your Leao application');
    ...
    try {
      ...
      logger.fatal('Failed to start your Leao application');
    }
  }
}
