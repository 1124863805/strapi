...
const getProviderSettings = (): EmailConfig => leao.config.get('plugin::email');

const send = async (options: SendOptions) => leao.plugin('email').provider.send(options);
...
const sendTemplatedEmail = (
  ...
  return leao.plugin('email').provider.send({ ...emailOptions, ...templatedAttributes });
};
