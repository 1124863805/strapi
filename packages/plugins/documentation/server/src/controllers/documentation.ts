...
export default {
  ...
  async loginView(ctx: Koa.Context, next: Koa.Next) {
    ...
    try {
      ...
      const filledLayout = _.template(layout.toString())({
        actionUrl: `${leao.config.server.url}/documentation/login`,
      });
  ...
  async login(ctx: Koa.Context) {
    ...
    const { password: hash } = (await leao
    ...
    ctx.redirect(`${leao.config.server.url}/documentation${querystring}`);
  },
  ...
  async updateSettings(ctx: Koa.Context) {
    const pluginStore = leao.store({ type: 'plugin', name: 'documentation' });
