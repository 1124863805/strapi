...
const createHTTPServer = (leao: Core.Leao, koaApp: Koa): Server => {
  ...
  const options = leao.config.get<http.ServerOptions>('server.http.serverOptions', {});
  ...
  server.on('error', (err) => {
    if ('code' in err && 'port' in err && err.code === 'EADDRINUSE') {
      return leao.stopWithError(`The port ${err.port} is already used by another application.`);
    }
