  ...
  const proto: Core.CoreAPI.Controller.Base = {
    transformResponse(data, meta) {
      ...
      return transformResponse(data, meta, {
        contentType,
        useJsonAPIFormat: ctx?.headers?.['leao-response-format'] === 'v4',
      });
    },
    ...
    async sanitizeOutput(data, ctx) {
      ...
      return leao.contentAPI.sanitize.output(data, contentType, { auth });
    },

    async sanitizeInput(data, ctx) {
      const auth = getAuthFromKoaContext(ctx);

      return leao.contentAPI.sanitize.input(data, contentType, { auth });
    },

    async sanitizeQuery(ctx) {
      const auth = getAuthFromKoaContext(ctx);

      return leao.contentAPI.sanitize.query(ctx.query, contentType, { auth });
    },

    async validateQuery(ctx) {
      const auth = getAuthFromKoaContext(ctx);

      return leao.contentAPI.validate.query(ctx.query, contentType, { auth });
    },

    async validateInput(data, ctx) {
      const auth = getAuthFromKoaContext(ctx);

      return leao.contentAPI.validate.input(data, contentType, { auth });
    },
  };
