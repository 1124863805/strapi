...
async function migrateReviewWorkflowStagesRoles({ oldContentTypes, contentTypes }: any) {
  ...
  // This ensures consistent behaviour when upgrading to a leao version with review workflows RBAC.
  if (!hadRolePermissions && hasRolePermissions) {
    ...
    const stages = await leao.db.query(STAGE_MODEL_UID).findMany();
    const roles = await leao.db.query(roleUID).findMany();
