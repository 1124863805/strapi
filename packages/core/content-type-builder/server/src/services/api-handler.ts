...
export async function clear(uid: Internal.UID.ContentType) {
  ...
  const { apiName, modelName } = leao.contentTypes[uid] as any;

  const apiFolder = path.join(leao.dirs.app.api, apiName);
...
export async function backup(uid: Internal.UID.ContentType) {
  const { apiName } = leao.contentTypes[uid] as any;

  const apiFolder = path.join(leao.dirs.app.api, apiName);
  const backupFolder = path.join(leao.dirs.app.api, '.backup', apiName);
...
async function deleteBackup(uid: Internal.UID.ContentType) {
  ...
  const backupFolder = path.join(leao.dirs.app.api, '.backup');
  const apiBackupFolder = path.join(leao.dirs.app.api, '.backup', apiName);
