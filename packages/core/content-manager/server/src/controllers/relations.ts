...
const addStatusToRelations = async (targetUid: UID.Schema, relations: RelationEntity[]) => {
  if (!contentTypes.hasDraftAndPublish(leao.getModel(targetUid))) {
    return relations;
  }
  ...
  const availableStatus = await leao.query(targetUid).findMany({
...
const validateLocale = (sourceUid: UID.Schema, targetUid: UID.ContentType, locale?: string) => {
  const sourceModel = leao.getModel(sourceUid);
  const targetModel = leao.getModel(targetUid);

  const isLocalized = leao.plugin('i18n').service('content-types').isLocalizedContentType;
...
export default {
  async extractAndValidateRequestInfo(ctx: any, id?: Data.ID) {
    ...
    const sourceSchema = leao.getModel(model);
    if (!sourceSchema) {
    ...
    if (id) {
      ...
      const currentEntity = await leao.db.query(model).findOne({
        where,
        populate,
      });
    ...
    const targetSchema = leao.getModel(targetUid);
  ...
  async findAvailable(ctx: any) {
    ...
    if (id) {
       ...
       */
      const subQuery = leao.db.queryBuilder(sourceUid);
    ...
    const dbQuery = leao.get('query-params').transform(targetUid, queryParams);

    const res = await leao.db.query(targetUid).findPage(dbQuery);
  ...
  async findExisting(ctx: any) {
     ...
     */
    const dbQuery = leao.db.query(sourceUid);
    ...
    const sanitizedRes = await loadRelations({ id: entryId }, targetField, {
      ...leao.get('query-params').transform(targetUid, permissionQuery),
      ordering: 'desc',
    });
