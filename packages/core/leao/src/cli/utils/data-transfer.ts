...
const setSignalHandler = async (
  ...
  signals.forEach((signal) => {
    // We specifically remove ALL listeners because we have to clear the one added in Leao bootstrap that has a process.exit
    // TODO: Ideally Leao bootstrap would not add that listener, and then this could be more flexible and add/remove only what it needs to
    process.removeAllListeners(signal);
    process.on(signal, handler);
  });
};

const createLeaoInstance = async (opts: { logLevel?: string } = {}): Promise<Core.Leao> => {
  try {
    const appContext = await compileLeao();
    const app = createLeao({ ...opts, ...appContext });

    app.log.level = opts.logLevel || 'error';
    return await app.load();
  } catch (error) {
    if (error instanceof Error && 'code' in error && error.code === 'ECONNREFUSED') {
      throw new Error('Process failed. Check the database connection with your Leao project.');
    }
...
const getDiffHandler = (
  ...
  return async (
    ...
    setSignalHandler(async () => {
      await abortTransfer({ engine, leao: leao as Core.Leao });
    ...
    Object.entries(context.diffs).forEach(([uid, diffs]) => {
      for (const diff of diffs) {
        ...
        if (
          ...
          endPath?.startsWith('leao_stage') ||
          endPath?.startsWith('leao_assignee')
        ) {
...
const getAssetsBackupHandler = (
  ...
  return async (
    ...
    console.warn(
      'The backup for the assets could not be created inside the public directory. Ensure Leao has write permissions on the public directory.'
    );
    const confirmed = await confirmMessage(
...
type RestoreConfig = NonNullable<
  leaoDataTransfer.providers.ILocalLeaoDestinationProviderOptions['restore']
>;
...
const parseRestoreFromOptions = (opts: Partial<engineDataTransfer.ITransferEngineOptions>) => {
  ...
  const restoreConfig: leaoDataTransfer.providers.ILocalLeaoDestinationProviderOptions['restore'] =
    {
