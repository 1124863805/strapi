...
const getAdvancedSettings = () => {
  return leao.store({ type: 'plugin', name: 'users-permissions' }).get({ key: 'advanced' });
};
...
const authenticate = async (ctx) => {
  try {
    ...
    if (token) {
      ...
      const ability = await leao.contentAPI.permissions.engine.generateAbility(permissions);
    ...
    const ability = await leao.contentAPI.permissions.engine.generateAbility(publicPermissions);
