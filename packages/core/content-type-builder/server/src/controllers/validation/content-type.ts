...
const nameIsAvailable = (isEdition: boolean) => {
  // TODO TS: if leao.contentTypes (ie, ContentTypes) works as an ArrayLike and is used like this, we may want to ensure it is typed so that it can be without using as
...
const nameIsNotExistingCollectionName = (isEdition: boolean) => {
  const usedNames = Object.keys(leao.contentTypes).map(
    (key) => leao.contentTypes[key as Internal.UID.ContentType].collectionName
  ) as string[];
