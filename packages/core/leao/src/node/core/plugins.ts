...
interface LeaoPlugin extends PackageJson {
  leao: {
    description?: string;
    displayName?: string;
    kind: 'plugin';
...
const validatePackageHasLeao = (
  pkg: PackageJson
): pkg is PackageJson & { leao: Record<string, unknown> } =>
  'leao' in pkg &&
  typeof pkg.leao === 'object' &&
  !Array.isArray(pkg.leao) &&
  pkg.leao !== null;

const validatePackageIsPlugin = (pkg: PackageJson): pkg is LeaoPlugin =>
  validatePackageHasLeao(pkg) && pkg.leao.kind === 'plugin';
...
const getEnabledPlugins = async ({
  ...
  runtimeDir,
  leao,
}: Pick<BaseContext, 'cwd' | 'logger' | 'leao' | 'runtimeDir'>): Promise<
...
> => {
   ...
   * are plugins.
   */
  const deps = leao.config.get('info.dependencies', {});
  ...
  for (const dep of Object.keys(deps)) {
    ...
    if (pkg && validatePackageIsPlugin(pkg)) {
      const name = pkg.leao.name || pkg.name;
  ...
  const userPluginsFile = await loadUserPluginsFile(leao.dirs.app.config);
...
const getMapOfPluginsWithAdmin = (plugins: Record<string, PluginMeta>) => {
   ...
   * package.json exports or from the legacy leao-admin.js file.
   */
  ...
  return Object.values(plugins)
    .filter((plugin) => {
      ...
      /**
       * There are two ways a plugin should be imported, either it's local to the leao app,
       * or it's an actual npm module that's installed and resolved via node_modules.
       *
       * We first check if the plugin is local to the leao app, using a regular `fs.existsSync` because
       * the pathToPlugin will be relative i.e. `/Users/my-name/leao-app/src/plugins/my-plugin`.
       *
      ...
      try {
        ...
        if (localPluginPath) {
          ...
          if (fs.existsSync(packageJsonPath)) {
            ...
            const localAdminPath = packageJson?.exports?.['./leao-admin']?.import;
          ...
          if (fs.existsSync(path.join(localPluginPath, 'leao-admin.js'))) {
            pluginImportPaths[plugin.modulePath] = 'leao-admin';
            return true;
          }
        }

        // This plugin is a module, so we need to check if it has a leao-admin export
        if (require.resolve(`${plugin.modulePath}/leao-admin`)) {
          pluginImportPaths[plugin.modulePath] = 'leao-admin';
          return true;
        }
