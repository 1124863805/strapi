 ...
 */
const getInitLocale = () => {
  const envLocaleCode = process.env.LEAO_PLUGIN_I18N_INIT_LOCALE_CODE;
  ...
  if (envLocaleCode) {
    ...
    if (!matchingLocale) {
      throw new Error(
        'Unknown locale code provided in the environment variable LEAO_PLUGIN_I18N_INIT_LOCALE_CODE'
      );
    }
