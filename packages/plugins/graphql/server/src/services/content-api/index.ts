...
export default ({ leao }: { leao: Core.Leao }) => {
  ...
  const buildSchema = () => {
    ...
    registerScalars({ registry, leao });
    registerInternals({ registry, leao });
    ...
    const currentEnv = leao.config.get('environment');
    ...
    const wrappedNexusSchema = wrapResolvers({ schema: nexusSchema, leao, extension });
  ...
  const shadowCRUD = () => {
    ...
    // Get every content type & component defined in Leao
    const contentTypes = [
      ...Object.values(leao.components),
      ...Object.values(leao.contentTypes),
    ];
  ...
  const registerAPITypes = (contentTypes: Struct.Schema[]) => {
    for (const contentType of contentTypes) {
      ...
      const registerOptions = { registry, leao, builders };
  ...
  const registerMorphTypes = (contentTypes: Struct.Schema[]) => {
    ...
    for (const contentType of contentTypes) {
      registerPolymorphicContentType(contentType, { registry, leao });
    }
  };
