...
export default {
  async deleteMany(ctx: Context) {
    ...
    const pmFolder = leao.service('admin::permission').createPermissionsManager({
      ability: ctx.state.userAbility,
      model: FOLDER_MODEL_UID,
    });

    const pmFile = leao.service('admin::permission').createPermissionsManager({
      ability: userAbility,
      action: ACTIONS.read,
  ...
  async moveMany(ctx: Context) {
    ...
    try {
      // fetch folders
      const existingFolders = await leao.db
      ...
      // fetch files
      const existingFiles = await leao.db
      ...
      // @ts-expect-error - no dynamic typings for the models
      const pathColName = leao.db.metadata.get(FOLDER_MODEL_UID).attributes.path.columnName;
      ...
      if (existingFolders.length > 0) {
        ...
        for (const existingFolder of existingFolders) {
          ...
          totalFolderNumber = await leao.db
            ...
            .update(
              pathColName,
              leao.db.connection.raw(replaceQuery, [
      ...
      if (existingFiles.length > 0) {
        ...
        // @ts-expect-error - no dynamic typings for the models
        const fileJoinTable = leao.db.metadata.get(FILE_MODEL_UID).attributes.folder.joinTable;
        await leao.db
    ...
    const updatedFolders = await leao.db.query(FOLDER_MODEL_UID).findMany({
      where: { id: { $in: folderIds } },
    });
