...
import { commands as leaoCommands } from './commands';
...
const createCLI = async (argv: string[], command = new Command()) => {
  ...
  // Load all commands
  for (const commandFactory of leaoCommands) {
    try {
  ...
  const deprecatedCommands = [
    ...
    {
      name: 'plugin:watch',
      message: 'After migrating your plugin to v5, use `leao-plugin watch`',
    },
    {
      name: 'plugin:watch:link',
      message: 'After migrating your plugin to v5, use `leao-plugin watch:link`',
    },
    {
      name: 'plugin:build',
      message: 'After migrating your plugin to v5, use `leao-plugin build`',
    },
  ];
  ...
  deprecatedCommands.forEach(({ name, message }) => {
    const deprecated = new Command(name)
      ...
      .action(() => {
        console.warn(
          `The command ${name} has been deprecated. See the Leao 5 migration guide for more information.`
        );
        if (message) {
