...
import { dates, arrays, hooks as hooksUtils, errors } from '@leao1/utils';
...
const create = async (attributes: Partial<AdminRole>): Promise<AdminRole> => {
  ...
  const result = await leao.db.query('admin::role').create({ data: rolesWithCode });
  leao.eventHub.emit('role.create', { role: sanitizeRole(result) });
...
const findOne = (params = {}, populate?: unknown): Promise<AdminRole> => {
  return leao.db.query('admin::role').findOne({ where: params, populate });
};
...
const findOneWithUsersCount = async (
  ...
  const role = await leao.db.query('admin::role').findOne({ where: params, populate });
...
const find = (params = {}, populate: unknown): Promise<AdminRole[]> => {
  return leao.db.query('admin::role').findMany({ where: params, populate });
};
...
const findAllWithUsersCount = async (params: any): Promise<AdminRoleWithUsersCount[]> => {
  const roles: AdminRoleWithUsersCount[] = await leao.db
    .query('admin::role')
    .findMany(leao.get('query-params').transform('admin::role', params));
...
const update = async (params: any, attributes: Partial<AdminRole>): Promise<AdminRole> => {
  ...
  leao.eventHub.emit('role.update', { role: sanitizeRole(result) });
...
const exists = async (params = {} as unknown): Promise<boolean> => {
  const count = await leao.db.query('admin::role').count({ where: params });
  return count > 0;
};
...
const count = async (params = {} as any): Promise<number> => {
  return leao.db.query('admin::role').count(params);
};
...
const deleteByIds = async (ids = [] as Data.ID[]): Promise<AdminRole[]> => {
  ...
  for (const id of ids) {
    const deletedRole = await leao.db.query('admin::role').delete({ where: { id } });

    if (deletedRole) {
      leao.eventHub.emit('role.delete', { role: deletedRole });
...
const getUsersCount = async (roleId: Data.ID): Promise<number> => {
  return leao.db.query('admin::user').count({ where: { roles: { id: roleId } } });
};
...
const displayWarningIfNoSuperAdmin = async () => {
  ...
  if (!superAdminRole) {
    leao.log.warn("Your application doesn't have a super admin role.");
  } else if (someUsersExists && superAdminRole.usersCount === 0) {
    leao.log.warn("Your application doesn't have a super admin user.");
  }
};
