...
module.exports = ({ leao }) => ({
  ...
  count(params) {
    return leao.db.query(USER_MODEL_UID).count({ where: params });
  },
  ...
  async ensureHashedPasswords(values) {
    const attributes = leao.getModel(USER_MODEL_UID).attributes;
   ...
   */
  async add(values) {
    return leao.db.query(USER_MODEL_UID).create({
  ...
  async edit(userId, params = {}) {
    return leao.db.query(USER_MODEL_UID).update({
      where: { id: userId },
   ...
   */
  fetch(id, params) {
    const query = leao.get('query-params').transform(USER_MODEL_UID, params ?? {});

    return leao.db.query(USER_MODEL_UID).findOne({
      ...query,
      where: {
        $and: [{ id }, query.where || {}],
      },
  ...
  fetchAuthenticatedUser(id) {
    return leao.db.query(USER_MODEL_UID).findOne({ where: { id }, populate: ['role'] });
  },
  ...
  fetchAll(params) {
    ...
    return leao.db.query(USER_MODEL_UID).findMany(query);
  },
   ...
   */
  async remove(params) {
    return leao.db.query(USER_MODEL_UID).delete({ where: params });
  },
  ...
  async sendConfirmationEmail(user) {
    ...
    const userSchema = leao.getModel(USER_MODEL_UID);
    ...
    try {
      settings.message = await userPermissionService.template(settings.message, {
        URL: urlJoin(
          leao.config.get('server.absoluteUrl'),
          apiPrefix,
