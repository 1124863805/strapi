...
const enableI18n = async ({ oldContentTypes, contentTypes }: Input) => {
  const { isLocalizedContentType } = leao.plugin('i18n')?.service('content-types') ?? {};
  const { getDefaultLocale } = leao.plugin('i18n')?.service('locales') ?? {};
  ...
  for (const uid in contentTypes) {
    ...
    if (!isLocalizedContentType(oldContentType) && isLocalizedContentType(contentType)) {
      ...
      await leao.db.query(uid).updateMany({
        where: { locale: null },
