...
const getDisabledUserList = async () => {
  return leao.store.get({ type: 'ee', key: 'disabled_users' });
};
...
const enableMaximumUserCount = async (numberOfUsersToEnable: number) => {
  ...
  await leao.db.query('admin::user').updateMany({
...
const disableUsersAboveSeatLimit = async (numberOfUsersToDisable: number) => {
  ...
  const nonSuperAdminUsersToDisable = await leao.db.query('admin::user').findMany({
    where: {
      isActive: true,
      roles: {
  ...
  if (nonSuperAdminUsersToDisable.length < numberOfUsersToDisable) {
    const superAdminUsersToDisable = await leao.db.query('admin::user').findMany({
      where: {
        isActive: true,
...
const seatEnforcementWorkflow = async () => {
  const adminSeats = leao.ee.seats;
  if (isNil(adminSeats)) {
    return;
  }
