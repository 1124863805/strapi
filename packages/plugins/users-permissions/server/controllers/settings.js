...
const { ValidationError } = require('@leao1/utils').errors;
const { getService } = require('../utils');
const { isValidEmailTemplate } = require('./validation/email-template');

module.exports = {
  async getEmailTemplate(ctx) {
    ctx.send(await leao.store({ type: 'plugin', name: 'users-permissions', key: 'email' }).get());
  },
  ...
  async getProviders(ctx) {
    ...
    for (const provider in providers) {
      if (provider !== 'email') {
        providers[provider].redirectUri = leao
