...
const createReleaseValidationService = ({ leao }: { leao: Core.Leao }) => ({
  async validateUniqueEntry(
     ...
     * Possibly related to the comment on GetValues: https://github.com/leao/leao/blob/main/packages/core/types/src/modules/entity-service/result.ts
     */
  ...
  validateEntryData(
    ...
    const contentType = leao.contentType(contentTypeUid as UID.ContentType);
  ...
  async validatePendingReleasesLimit() {
    ...
    const featureCfg = leao.ee.features.get('cms-content-releases');

    const maximumPendingReleases =
      (typeof featureCfg === 'object' && featureCfg?.options?.maximumReleases) || 3;

    const [, pendingReleasesCount] = await leao.db.query(RELEASE_MODEL_UID).findWithCount({
      filters: {
        releasedAt: {
          $null: true,
        },
  ...
  async validateUniqueNameForPendingRelease(
    ...
    const pendingReleases = (await leao.db.query(RELEASE_MODEL_UID).findMany({
      where: {
        releasedAt: {
          $null: true,
        },
