...
const createEntityQuery = (leao: Core.Leao): any => {
  const components = {
  ...
  const query = (uid: UID.Schema) => {
    const create = async <T extends { data: U }, U extends object>(params: T) => {
      ...
      return leao.db.query(uid).create({ ...params, data: sanitizedData });
    };
    ...
    const createMany = async <T extends { data: U[] }, U extends object>(params: T) => {
      return (
        Promise.resolve(params.data)
          ...
          // Execute a leao db createMany query with all the entities + their created components
          .then((data) => leao.db.query(uid).createMany({ ...params, data }))
      );
    };

    const deleteMany = async <T extends object>(params?: T) => {
      const entitiesToDelete = await leao.db.query(uid).findMany(params ?? {});
      ...
      const deletedEntities = await leao.db.query(uid).deleteMany(params);
      ...
      for (const [key, attribute] of entries) {
        ...
        if (attribute.type === 'dynamiczone') {
          ...
          for (const componentUID of componentsUID) {
            const component = leao.getModel(componentUID);
            const subPopulate = getDeepPopulateComponentLikeQuery(component, params);
    ...
    return {
      ...
      get deepPopulateComponentLikeQuery() {
        const contentType = leao.getModel(uid);
