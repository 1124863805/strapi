...
export const installGraphqlExtension = ({ leao }: { leao: Core.Leao }) => {
  const { service: getGraphQLService, config: graphQLConfig } = leao.plugin('graphql');
  const { service: getUploadService } = leao.plugin('upload');
  ...
  const fileModel = leao.getModel(FILE_MODEL_UID);
