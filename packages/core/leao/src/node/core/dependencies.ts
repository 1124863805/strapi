...
/**
 * From V5 this will be imported from the package.json of `@leao1/leao`.
 */
const PEER_DEPS = {
  react: '^18.0.0',
  'react-dom': '^18.0.0',
 ...
 * needed by the Leao admin project. Whilst generally speaking most modules will be
...
const checkRequiredDependencies = async ({
  ...
  if (process.env.USE_EXPERIMENTAL_DEPENDENCIES === 'true') {
    logger.warn('You are using experimental dependencies that may not be compatible with Leao.');
  ...
  if (install.length > 0) {
    logger.info(
      'The Leao admin needs to install the following dependencies:',
      os.EOL,
    ...
    /**
     * Re-run the same command after installation e.g. leao build because the yarn.lock might
  ...
  if (review.length) {
    ...
    for (const dep of review) {
      ...
      if (!minDeclaredVersion) {
        ...
        logger.warn(
          [
            `Declared version of ${dep.name} (${minDeclaredVersion}) is not compatible with the version required by Leao (${dep.wantedVersion}).`,
      ...
      if (!installedVersion) {
        ...
        logger.warn(
          [
            `Declared version of ${dep.name} (${installedVersion}) is not compatible with the version required by Leao (${dep.wantedVersion}).`,
            'You may experience issues, we recommend you change this.',
          ].join(os.EOL)
        );
      }
