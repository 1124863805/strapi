...
import { errors, contentTypes as contentTypeUtils } from '@leao1/utils';
    ...
    if (attr.type === 'dynamiczone' && isArray(value)) {
      value.forEach((compo) => {
        if (has('__component', compo)) {
          const model = leao.components[compo.__component];
          removeIdsMut(model, compo);
        }
      });
    } else if (attr.type === 'component') {
      const model = leao.components[attr.component];
      if (isArray(value)) {
...
const fillNonLocalizedAttributes = (entry: any, relatedEntry: any, { model }: any) => {
  ...
  const modelDef = leao.getModel(model);
...
const getNestedPopulateOfNonLocalizedAttributes = (modelUID: any) => {
  const schema = leao.getModel(modelUID);
