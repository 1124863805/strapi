...
const command = () => {
  return (
    createCommand('import')
      .description('Import data from file to Leao')
      .allowExcessArguments(false)
      .requiredOption(
        '-f, --file <file>',
        'path and filename for the Leao export file you want to import'
      )
      .addOption(
        new Option(
          '-k, --key <string>',
      ...
      .hook('preAction', (thisCommand) => {
        ...
        if (extname(file) !== '.tar') {
          exitWith(
            1,
            `The file '${opts.file}' does not appear to be a valid Leao data file. It must have an extension ending in .tar[.gz][.enc]`
          );
        }
      })
      .hook(
