...
const readPackageJSON = async (path: string) => {
  try {
    ...
    const uuid = packageObj.leao ? packageObj.leao.uuid : null;
...
const action = async () => {
  ...
  if ((packageObj.leao && packageObj.leao.telemetryDisabled) || !uuid) {
  ...
  if (!write) {
    console.log(
      `${chalk.yellow(
        'Warning'
      )}: There has been an error, please set "telemetryDisabled": true in the "leao" object of your package.json manually.`
    );
    process.exit(0);
  }

  await sendEvent('didOptOutTelemetry', uuid);
  console.log(`${chalk.green('Successfully opted out of Leao telemetry')}`);
  process.exit(0);
};

/**
 * `$ leao telemetry:disable`
 */
const command: LeaoCommand = () => {
  return createCommand('telemetry:disable')
    .description('Disable anonymous telemetry and metadata sending to Leao analytics')
