...
const keyIsValidEventName = ([key]: any) => {
  return Object.keys(leao.service('admin::passport').authEventsMapper).includes(key);
};

const getPassportStrategies = () => [createLocalStrategy(leao)] as Strategy[];

const registerAuthEvents = () => {
  // @ts-expect-error - TODO: migrate auth service to TS
  const { events = {} } = leao.config.get('admin.auth', {});
  const { authEventsMapper } = leao.service('admin::passport');
  ...
  for (const [eventName, handler] of eventList) {
    ...
    leao.eventHub.on(authEventsMapper[eventName], handler as any);
  }
};
