...
import { darkTheme, lightTheme } from '@leao/design-system';
...
import Logo from './assets/images/logo-leao-2022.svg';
...
import { Router, LeaoAppSetting, UnloadedSettingsLink } from './core/apis/router';
...
interface LeaoAppConstructorArgs extends Partial<Pick<LeaoApp, 'appPlugins'>> {
  config?: {
    auth?: { logo: string };
    head?: { favicon: string };
    locales?: string[];
...
interface LeaoAppPlugin {
  bootstrap?: (
    args: Pick<LeaoApp, 'addSettingsLink' | 'addSettingsLinks' | 'getPlugin' | 'registerHook'>
  ) => void;
  register: (app: LeaoApp) => void;
...
class LeaoApp {
  appPlugins: Record<string, LeaoAppPlugin>;
  ...
  translations: LeaoApp['configurations']['translations'] = {};
  ...
  constructor({ config, appPlugins }: LeaoAppConstructorArgs = {}) {
  ...
  addMiddlewares = (middlewares: LeaoApp['middlewares']) => {
  ...
  createSettingSection = (
    section: Pick<LeaoAppSetting, 'id' | 'intlLabel'>,
  ...
  createCustomConfigurations = (customConfig: NonNullable<LeaoAppConstructorArgs['config']>) => {
...
export { LeaoApp };
export type { LeaoAppPlugin, LeaoAppConstructorArgs, InjectionZoneComponent };
