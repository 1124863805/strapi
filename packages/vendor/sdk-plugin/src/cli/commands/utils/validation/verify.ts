 ...
 * 2. Has ./leao-admin and/or ./leao-server exports
...
export const verify = async ({ cwd, logger }: VerifyOptions) => {
   ...
   */
  if (!validatedPkg.exports?.['./leao-admin'] && !validatedPkg.exports?.['./leao-server']) {
    packageJsonLoader.fail();
    throw new Error(
      'You need to have either a leao-admin or leao-server export in your package.json'
    );
  }
