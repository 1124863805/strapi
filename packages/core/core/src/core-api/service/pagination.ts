...
import { errors, pagination } from '@leao1/utils';
...
const getLimitConfigDefaults = () => ({
  defaultLimit: toNumber(leao.config.get('api.rest.defaultLimit', 25)),
  maxLimit: toNumber(leao.config.get('api.rest.maxLimit')) || null,
});
...
const shouldCount = (params: { pagination?: PaginationParams }) => {
  ...
  return Boolean(leao.config.get('api.rest.withCount', true));
};
